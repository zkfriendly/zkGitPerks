...
// regex: <span>Payment of the total price of this journey, =\r\namounting to [0-9]+
template TotalChargedRegex(msg_bytes) {
	...
	component and[68][num_bytes];
	component multi_or[2][num_bytes];
	signal states[num_bytes+1][68];
	...
	for (var i = 0; i < num_bytes; i++) {
		state_changed[i] = MultiOR(67);
		eq[0][i] = IsEqual();
		eq[0][i].in[0] <== in[i];
		eq[0][i].in[1] <== 108;
		and[0][i] = AND();
		and[0][i].a <== states[i][67];
		...
		multi_or[0][i].in[0] <== eq[24][i].out;
		multi_or[0][i].in[1] <== eq[25][i].out;
		multi_or[0][i].in[2] <== eq[26][i].out;
		multi_or[0][i].in[3] <== eq[27][i].out;
		multi_or[0][i].in[4] <== eq[28][i].out;
		multi_or[0][i].in[5] <== eq[29][i].out;
		...
		multi_or[1][i].in[0] <== and[42][i].out;
		multi_or[1][i].in[1] <== and[43][i].out;
		states[i+1][43] <== multi_or[1][i].out;
	...
	for (var i = 0; i < msg_bytes; i++) {
		is_substr0[i][0] <== 0;
		is_substr0[i][1] <== is_substr0[i][0] + states[i+1][42] * states[i+2][43];
		is_substr0[i][2] <== is_substr0[i][1] + states[i+1][43] * states[i+2][43];
