...
import { GateKeeper, IERC20__factory, Tum, TumVerifier } from "../build/typechain"
...
import tumProof from "./sample_proof/tum_proof.json"
...
describe("Tum", async () => {
    ...
    beforeEach(async () => {
        ...
        const TumVerifierFactory = await ethers.getContractFactory("TumVerifier")
        tumVerifier = (await TumVerifierFactory.deploy()) as TumVerifier

        const TumFactory = await ethers.getContractFactory("Tum")
        tumContract = await TumFactory.deploy(
    ...
    it("should have proper setup", async () => {
        expect(tumContract.address).to.be.properAddress
        expect(await tumContract.gateKeeper()).to.equal(gateKeeperContract.address)
        expect(await tumContract.token()).to.equal(token.address)
        expect(await tumContract.ticketPrice()).to.equal(ticketPrice)
    })

    it("should be able to claim with valid proof", async () => {
        await joinContributors(prProof.calldata, user.commitment)
        const rawSignal = ethers.utils.keccak256(tumContract.address)
        const scope = await tumContract.getScope()
        ...
        //@ts-ignore
        await tumContract.connect(signer).claim(gateKeeperProof, tumProof.calldata)
    })
})
