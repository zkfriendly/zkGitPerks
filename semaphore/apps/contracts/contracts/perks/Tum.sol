...
import "../interfaces/ITumVerifier.sol";

interface IERC20 {
    function transfer(address to, uint256 amount) external returns (bool);
}

/// @title Tum Blockchain conference ticket reimbursement contract
/// @author zkfriendly
/// @notice This contract is used to reimburse the ticket of the blockchain conference based on a given gatekeeper
contract Tum {
    ...
    function claim(GateKeeperProof calldata gateKeeperProof, Proof calldata proof) external {
        ...
        if (!ITumVerifier(groth16verifier).verifyProof(proof._pA, proof._pB, proof._pC, proof._pubSignals))
            revert InvalidProof();
