...
template DBVerifier(max_header_bytes, max_body_bytes, n, k, pack_size, max_total_charged_len) {
   ...
   // expose repo name
    signal (regex_out, total_charged_reveal[max_body_bytes]) <== TotalChargedRegex(max_body_bytes)(in_body_padded);
    log(regex_out);
    regex_out === 1;
...
component main = DBVerifier(2048, 276480, 121, 17, 31, 10);