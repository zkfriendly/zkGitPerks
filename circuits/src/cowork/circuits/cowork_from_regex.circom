...
// regex: Devconnect Cowork Space <cowork@ticketh.xyz>
template FromRegex(msg_bytes) {
	...
	component and[44][num_bytes];
	signal states[num_bytes+1][45];
	...
	for (var i = 0; i < num_bytes; i++) {
		state_changed[i] = MultiOR(44);
		eq[0][i] = IsEqual();
		eq[0][i].in[0] <== in[i];
		eq[0][i].in[1] <== 99;
		and[0][i] = AND();
		and[0][i].a <== states[i][44];
