...
// regex: \[Devconnect Cowork\] Payment received for your order
template PaymentRegex(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		state_changed[i] = MultiOR(51);
		eq[0][i] = IsEqual();
		eq[0][i].in[0] <== in[i];
		eq[0][i].in[1] <== 110;
		and[0][i] = AND();
		and[0][i].a <== states[i][51];
